...
%
% Revised 5/24 so that addscalc is called when the ELB is imposed
% but not violated and an asymmetric policy rule is used.
%
% *********************************************************************
...
for ireplic = 1:nreplic
   if rem(ireplic,100) == 0
    ...
    for iqtr = 1:nsimqtrs
        ...
        % funds rate 
        % 
        % (a) when the ELB constraint is imposed and either ELB
        % violations exist along the initial projected RFF path
        % or the policy rule is nonlinear. If the ECFS option
        % is selected, the addscalc routine also generates fiscal
        % shocks as needed to prevent the output gap from falling
        % below a specified level
        %
        % (b) when the ELB constraint is not imposed and the policy
        % rule is nonlinear
